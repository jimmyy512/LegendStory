...
PhysicsBody* MyBodyParser::bodyFormJson(cocos2d::Node *pNode, const std::string& name, PhysicsMaterial material,float border)
{
	PhysicsBody* body = nullptr;
	rapidjson::Value &bodies = doc["rigidBodies"];
	if (bodies.IsArray())
			...
			if (0 == strcmp(name.c_str(), bodies[i]["name"].GetString()))
			{
				...
				if (bd.IsObject())
				{
					...
					for (int i = 0; i<polygons.Size(); ++i)
					{
						...
						body->addShape(PhysicsShapePolygon::create(points,pcount, material, Vec2(0,0),5.f));
						delete[] points;
					}
				}
				else
